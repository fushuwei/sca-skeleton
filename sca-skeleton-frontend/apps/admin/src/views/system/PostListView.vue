...
const searchForm = reactive<PostPageRequest>({
  pageNum: 1,
  pageSize: 10,
  keyword: ""
});
...
const drawerTitle = computed(() => {
  ...
  if (drawerMode.value === "edit") return t("postMgmt.editPost");
  return t("postMgmt.viewPost");
});
...
function openPostDrawer(mode: DrawerMode, post?: SysPost) {
  drawerMode.value = mode;
  drawerPost.value = post;
...
// ── 表格列定义 ──
const columns = computed<QTableColumn<SysPost>[]>(() => [
  {
    name: "name",
    field: "name",
...
async function loadTableData(
  ...
  try {
    const result = await getPostPageApi(params);
  ...
  <div class="post-list-shell">
    ...
    <div class="right-panel">
          ...
          <div class="row q-col-gutter-sm items-end">
            <div class="col">
              <q-input
                ...
                square
                dense
                :placeholder="t('postMgmt.keywordPlaceholder')"
                hide-bottom-space
                clearable
      ...
      <q-table
        ...
        selection="multiple"
        flat
        :class="['post-table', { 'post-table--empty': !tableRows.length }]"
        ...
        <div class="post-local-drawer">
          <div class="post-drawer-shell">
            <div class="post-drawer-header row items-center no-wrap">
              ...
              <span class="post-drawer-title">{{ drawerTitle }}</span>
              <q-space />
              <q-btn
                flat
                dense
...
.post-table :deep(.q-table__top) {
  display: none;
}

.post-table :deep(.q-table__container) {
  display: flex;
  flex-direction: column;
  height: 100%;
}

.post-table :deep(.q-table__middle) {
  flex: 1 1 0;
  min-height: 0;
  overflow: auto;
  display: flex;
...
.post-table :deep(thead) {
  position: sticky;
  top: 0;
  z-index: 2;
}

.post-table :deep(.q-table__middle > table) {
  flex: 0 0 auto;
}

.post-table :deep(thead tr th) {
...
.post-table :deep(thead tr:first-child th) {
  border-top: none;
}

/* ── 空数据状态 ── */
.post-table--empty :deep(.q-table__container) {
  height: 100%;
}

.post-table--empty :deep(.q-table__middle) {
  flex: 0 0 auto;
  overflow: visible;
}

.post-table--empty :deep(.q-table__bottom) {
  flex: 1 1 0;
  min-height: 0;
  display: flex;
  align-items: center;
...
.post-table--empty :deep(.q-table__bottom .q-table__bottom-nodata-icon) {
  display: none;
}

/* 行悬停 */
.post-table :deep(tbody tr:hover td) {
  background: rgba(0, 121, 107, 0.03) !important;
}

.post-table :deep(tbody tr.q-tr--selected td) {
  background: rgba(0, 121, 107, 0.06) !important;
}

.post-table :deep(tbody td) {
  font-size: 13px;
...
/* 分页底栏 */
.post-table :deep(.q-table__bottom) {
...
/* 复选框尺寸 */
.post-table :deep(.q-checkbox__inner) {
  font-size: 32px;
}

/* ═══ 本地右侧抽屉 ═══ */
.post-local-drawer-mask {
  position: fixed;
  inset: 0;
  z-index: 5000;
...
.post-drawer-close-btn :deep(.q-btn__wrapper) {
  min-width: 32px;
  min-height: 32px;
  padding: 0;
}

.post-drawer-close-btn :deep(.q-icon) {
  font-size: 20px;
}

.post-drawer-close-btn:hover {
  background: rgba(128, 128, 128, 0.2);
}

/* 内容区容器：本身不滚动（三段式布局），滚动职责下沉到内部 .post-drawer-main */
.post-drawer-body {
  flex: 1 1 auto;
  min-height: 0;
  display: flex;
...
/* 抽屉滑入/滑出动画 */
.post-drawer-slide-enter-active,
.post-drawer-slide-leave-active {
  transition: opacity 0.25s ease;
}

.post-drawer-slide-enter-active .post-local-drawer,
.post-drawer-slide-leave-active .post-local-drawer {
  transition: transform 0.25s ease;
}

.post-drawer-slide-enter-from,
.post-drawer-slide-leave-to {
  opacity: 0;
}

.post-drawer-slide-enter-from .post-local-drawer,
.post-drawer-slide-leave-to .post-local-drawer {
...
/* 抽屉暗色模式 */
.body--dark .post-local-drawer {
  background: #1e1e1e !important;
  box-shadow: -4px 0 12px rgba(0, 0, 0, 0.4);
}

.body--dark .post-drawer-header {
  background: #252525 !important;
  border-bottom-color: rgba(255, 255, 255, 0.08) !important;
}

.body--dark .post-drawer-title {
  color: rgba(255, 255, 255, 0.87) !important;
}

.body--dark .post-drawer-close-btn {
  color: rgba(255, 255, 255, 0.6) !important;
}

.body--dark .post-drawer-close-btn:hover {
  background: rgba(255, 255, 255, 0.08) !important;
}

.body--dark .post-drawer-body {
  color: rgba(255, 255, 255, 0.87) !important;
}

.body--dark .post-local-drawer-mask {

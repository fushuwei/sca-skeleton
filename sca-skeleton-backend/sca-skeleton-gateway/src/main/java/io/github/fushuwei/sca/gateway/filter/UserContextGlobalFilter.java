...
public class UserContextGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        String traceId = exchange.getRequest().getHeaders()
                .getFirst(BaseConstants.HEADER_TRACE_ID);
                ...
                .switchIfEmpty(
                    ...
                    chain.filter(exchange.mutate()
                        .request(r -> r.headers(headers ->
                            headers.set(BaseConstants.HEADER_TRACE_ID, finalTraceId)))
                        .build())
                );
    }
        ...
        ServerHttpRequest mutatedRequest = exchange.getRequest().mutate()
                .headers(headers -> {
                    if (StringUtils.hasText(userId)) {
                        headers.set(BaseConstants.HEADER_USER_ID, userId);
                    }
                    if (StringUtils.hasText(username)) {
                        headers.set(BaseConstants.HEADER_USERNAME, username);
                    }
                    if (StringUtils.hasText(tenantId)) {
                        headers.set("X-Tenant-Id", tenantId);
                    }
                    headers.set(BaseConstants.HEADER_TRACE_ID, traceId);
                })
                .build();

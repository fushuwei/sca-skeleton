...
public class DriverStoreConfig {
     ...
     */
    @Bean
    @ConditionalOnProperty(prefix = "sca.datasource.driver", name = "storage-type", havingValue = "local", matchIfMissing = true)
     ...
     * MinIO 驱动存储实现 Bean
     *
     * @param minioUtils MinIO 工具类
     * @return MinioDriverStore 实例
     */
    @Bean
    @ConditionalOnProperty(prefix = "sca.datasource.driver", name = "storage-type", havingValue = "minio")
